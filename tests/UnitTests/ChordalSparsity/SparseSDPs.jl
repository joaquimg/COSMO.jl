module SparseSDPs
  ...
  function generateArrowMultCones(rng,m,numCones,nBlk,BlkSize,ArrowWidth)
  ...
  # generate random problem data based on pattern
    for iii=1:m
      ...
      for k= 1:numCones
        M = generateSparseSymmetricMatrix(rng,pattern[k],-100.,100.)
        Ai = [Ai;vec(M)]
      end
      A[:,iii] = Ai
    end
    ...
    for k=1:numCones
      Ytrue = generatePosDefMatrix(Ks[k],rng)
  ...
  function generateBandedSDP(rng::MersenneTwister,m::Int64,K,bandwidth::Array{Int64},aMin::Float64,aMax::Float64,density::Float64)
    ...
    numRows = K.f + K.l + sum(K.q) + sum(map(x->x^2,K.s))
    A = spzeros(numRows,m)
    for i = 1:m
      ...
      for k=1:length(K.s)
        Temp = randomBandedSymMatrix(rng,K.s[k],bandwidth[k],aMin,aMax)
        Ai = [Ai;vec(Temp)]
      end
      A[:,i] = Ai
    end
    ...
    for i=1:length(K.s)
      Temp = randomBandedSymMatrix(rng,K.s[i],bandwidth[i],aMin,aMax)
      Temp = Temp + (-minimum(eigs(Temp)[1])+1)*speye(K.s[i])
    ...
    for i=1:length(K.q)
     ...
     for i=1:length(K.s)
      Ytrue = generatePosDefMatrix(K.s[i],rng)
      ytrue = [ytrue;vec(Ytrue)]
    end
    q = (-P*xtrue -  A'*ytrue)[:]
