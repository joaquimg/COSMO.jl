...
for iii=1:length(results)
  ...
  for s in resData
    ...
    PyPlot.semilogy(xdata,s.solverTime[1:s.ind],color=cc[kkk],label=s.solverName,marker=markerList[kkk])
    kkk+=1
  end
  PyPlot.grid(true)
  PyPlot.xticks(xdata)
  ...
  PyPlot.savefig(dir*"/latex/$(timestamp)-$(resData[1].problemType).eps")
